...
try {
  for(const width of [1440,768,390]) {
    ...
    const errors=[];let editable=true, fail=false, payerFail=false, calls=0, payerCalls=0, requests=[], payerState='confirmation_required';
    const payerReview=()=>({state:payerState,missing_fields:payerState==='missing_data'?['tax_id','zip']:[],request_id:'e3000000-0000-0000-0000-000000000001',environment:'production',binding_id:payerState==='binding_required'?null:'e4000000-0000-0000-0000-000000000001',snapshot_id:payerState==='confirmed'?'e5000000-0000-0000-0000-000000000001':null,confirmed_at:payerState==='confirmed'?'2026-09-09T12:10:00Z':null,source_updated_at:'2026-09-09T11:00:00Z',source_fingerprint:'0123456789abcdef0123456789abcdef',seu_numero:'AP1234567890123',amount:1500,due_date:'2026-10-15',payer:{name:'Cliente de teste',tax_id:'12345678000195',type:'JURIDICA',email:null,ddd:'11',phone:'999999999',street:'Rua Teste',number:'10',complement:null,district:'Centro',city:'São Paulo',state:'SP',zip:'01001000'},dispatch_enabled:false,canConfirm:editable});
    ...
    await page.route('**/*',async route=>{
      ...
      if(url.includes('/_serverFn/')) {
        ...
        if(url.includes(prepareId)) {
          calls++;
          requests=[{id:'e3000000-0000-0000-0000-000000000001',environment:'production',amount:1500,due_date:'2026-10-15',status:'blocked_homologation',created_at:'2026-09-09T12:00:00Z',deleted_at:null}];
          return route.fulfill({json:{result:{id:'e3000000-0000-0000-0000-000000000001',reused:false},context:{}}});
        }
        if(url.includes(payerConfirmId)) {
          payerCalls++;payerState='confirmed';
          return route.fulfill({json:{result:{id:'e5000000-0000-0000-0000-000000000001',reused:false,dispatch_enabled:false},context:{}}});
        }
        if(url.includes(payerReviewId) && payerFail)return route.fulfill({json:{error:{message:'Falha do pagador simulada'},context:{}}});
        const result=url.includes(permissionsId)?[{module:'financeiro',action:'view'},...(editable?[{module:'financeiro',action:'edit'}]:[])]:url.includes(payerReviewId)?payerReview():url.includes(reviewId)?{receivable,canPrepare:editable,requests}:[];
    ...
    if(width===1440){
      payerFail=true;await dialog.getByRole('button',{name:'Atualizar pagador'}).click();
      await dialog.getByText('Dados do pagador indisponíveis',{exact:true}).waitFor();
      payerFail=false;await dialog.getByRole('button',{name:'Tentar novamente'}).click();
      await dialog.getByText('Vínculo bancário necessário',{exact:true}).waitFor();
    }
    ...
    requests[0].amount=1500;payerState='confirmation_required';
    editable=false;
    ...
    assert.equal(await dialog.getByRole('button',{name:'Registrar preparação',exact:true}).count(),0);
    await dialog.getByRole('tab',{name:'Produção / Oficial'}).click();
    await dialog.getByText('A confirmação exige perfil Admin ou Financeiro',{exact:false}).waitFor();
    assert.equal(await dialog.getByRole('button',{name:'Confirmar pagador',exact:true}).count(),0);
