...
const request = (
  body: unknown = { request_id: requestId, confirmed: true },
  bearer = "user-jwt",
) =>
...
Deno.test("emits sandbox charge with mTLS, required scopes and safe payload", async () => {
  ...
  const prepare = test.calls.find((call) =>
    call.url.includes("prepare_inter_sandbox")
  );
  const load = test.calls.find((call) =>
    call.url.includes("load_inter_sandbox")
  );
  ...
  const finish = test.calls.find((call) =>
    call.url.includes("finish_inter_sandbox")
  );
  assert(
