...
function DashboardPage() {
  ...
  const statusBreakdown = useMemo(
    () => [
      { status: "Abertos", count: tickets.filter((t) => t.status === "open").length, color: "#0EA5E9" },
      { status: "Em atendimento", count: tickets.filter((t) => t.status === "in_progress").length, color: "#8B5CF6" },
      { status: "Pendentes", count: tickets.filter((t) => t.status === "pending").length, color: "#F59E0B" },
      { status: "Resolvidos", count: tickets.filter((t) => t.status === "resolved").length, color: "#10B981" },
    ],
    [tickets],
  );

  const kpis = [
    { label: "Tickets abertos", value: open.length, icon: Inbox, color: "text-sky-600 bg-sky-50" },
    { label: "SLA em risco", value: slaAtRisk, icon: AlertTriangle, color: "text-amber-600 bg-amber-50" },
    { label: "MTTR (horas)", value: mttrHoras, icon: Clock, color: "text-violet-600 bg-violet-50" },
    { label: "Contratos ativos", value: contractsQ.data ?? 0, icon: FileCheck2, color: "text-emerald-600 bg-emerald-50" },
  ];
              ...
              {latest.map((t) => (
                <tr key={t.id} className="border-t">
                  ...
                  <td className="py-2 px-4">
                    <Link to="/tickets/$id" params={{ id: String(t.id) }} className="hover:underline">
                      {t.subject}
                    </Link>
                  </td>
