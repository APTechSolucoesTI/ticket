...
function guessField(header: string): string {
  ...
  const map: Record<string, string> = {
    nome: "name", identificacao: "name", "identificação": "name", equipamento: "name",
    cliente: "company_name", empresa: "company_name",
    contato: "contact_name",
    tipo: "type", marca: "brand", modelo: "model",
    serie: "serial_number", "série": "serial_number", "n° série": "serial_number", "numero de serie": "serial_number",
    patrimonio: "asset_tag", "patrimônio": "asset_tag",
    so: "operating_system", "sistema operacional": "operating_system",
    processador: "processor", cpu: "processor",
    memoria: "memory", "memória": "memory", ram: "memory",
    armazenamento: "storage", hd: "storage", ssd: "storage",
    localizacao: "location", "localização": "location",
    status: "status", observacoes: "notes", "observações": "notes",
    "data aquisicao": "purchase_date", "data de aquisição": "purchase_date", aquisicao: "purchase_date",
    garantia: "warranty_until", "garantia até": "warranty_until",
    "chave do sistema operacional": "os_key", "chave so": "os_key",
    "chave do office": "office_key", "chave office": "office_key",
  };
  return map[h] ?? NONE;
}
...
export function EquipmentImportDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (o: boolean) => void }) {
  ...
  function downloadTemplate() {
    const headersRow = [
      "Nome", "Cliente", "Contato", "Tipo", "Marca", "Modelo",
      "Número de Série", "Patrimônio", "Sistema Operacional", "Processador",
      "Memória", "Armazenamento", "Localização", "Status", "Observações",
      "Data de Aquisição", "Garantia até", "Chave do Sistema Operacional", "Chave do Office",
    ];
    const sample = [
      "Notebook Diretoria", "ACME LTDA", "João Silva", "Notebook", "Dell", "Latitude 5430",
      "SN123456", "PAT-001", "Windows 11 Pro", "i7-1260P",
      "16 GB", "SSD 512 GB", "Matriz - Sala 3", "active", "Equipamento em uso",
      "2024-01-15", "2027-01-15", "XXXXX-XXXXX-XXXXX-XXXXX-XXXXX", "YYYYY-YYYYY-YYYYY-YYYYY-YYYYY",
    ];
  ...
  const importMut = useMutation({
    mutationFn: async () => {
      ...
      const contactByKey = new Map((contacts ?? []).map((c) => [`${c.company_id}::${norm(c.name)}`, c.id]));
      ...
      rows.forEach((row, idx) => {
        ...
        for (const [col, field] of Object.entries(mapping)) {
          ...
          if (field === "company_name") companyName = val;
            ...
            const v = norm(val);
            rec.status = ["maintenance", "manutencao", "manutenção"].includes(v) ? "maintenance"
              : ["retired", "baixado", "inativo"].includes(v) ? "retired" : "active";
          } else rec[field] = val;
        }
    ...
    onSuccess: (r) => {
      toast.success(`${r.inserted} equipamento(s) importado(s)${r.skippedRows.length ? ` · ${r.skippedRows.length} ignorado(s)` : ""}`);
      qc.invalidateQueries({ queryKey: ["equipments"] });
      if (r.skippedRows.length) {
        setSkipped(r.skippedRows);
        setRows([]); setHeaders((h) => h); // keep headers for download
      } else {
