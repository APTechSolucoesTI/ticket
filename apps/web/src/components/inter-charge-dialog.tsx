    ...
    <Dialog open onOpenChange={(open) => !open && !mutation.isPending && onClose()}>
      <DialogContent className="max-w-2xl">
        ...
        <div className="flex items-start gap-2 rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-sm">
          ...
          <p>
            Envio ao banco bloqueado até a homologação. Preparar não emite boleto, não valida o
            pagador e não altera o status financeiro.
          </p>
        </div>
        {query.isPending ? (
          ...
          receivable && (
            <>
              ...
              {existing ? (
                ...
                >
                  <Badge variant="outline">Aguardando homologação</Badge>
                  <p className="text-sm">
